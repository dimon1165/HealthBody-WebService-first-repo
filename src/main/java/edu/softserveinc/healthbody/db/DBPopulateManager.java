...
public final class DBPopulateManager {

    private static volatile DBPopulateManager instance;
		...
		try (PreparedStatement pst = con.prepareStatement(query)) {
			for (int j = 1; j <= competitions; j++) {
				...
				Date startDate = new Date(System.currentTimeMillis()
						+ (long) (System.currentTimeMillis() / 2 == 0 ? 1 : -1)
						* new Random().nextInt(10) * 24 * 60 * 60 * 1000);
